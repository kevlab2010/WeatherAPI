...
public class OpenWeatherClient {
	...
	@Autowired
	public OpenWeatherClient(WebClient.Builder webClientBuilder, @Value("${org.openweathermap.api.baseurl}") String url) {
		this.webClientBuilder = webClientBuilder;
		this.url = url;
	}
	
	public WeatherForecastDto populateWeatherForecast(WeatherForecastDto dto) {
		
		  HttpHeaders headers = new HttpHeaders(); 
		  headers.set(HttpHeaders.ACCEPT,  MediaType.APPLICATION_JSON_VALUE);
		  
		  MultiValueMap<String, String> queryParams = new LinkedMultiValueMap<>();
		  queryParams.add("lat", Double.toString(dto.getLatitude()));
		  queryParams.add("lon", Double.toString(dto.getLongitude()));
		  queryParams.add("exclude", "current,minutely,daily,alerts");
		  ...
		  UriComponentsBuilder uriBuilder = UriComponentsBuilder 
			...
			OpenWeatherMapAPIResponse openWeatherResponse = webClientBuilder
			...
			.exchangeToMono( response -> {
				if (response.statusCode() == HttpStatus.OK)
					return response.bodyToMono(OpenWeatherMapAPIResponse.class);				
			...
			BeanUtils.copyProperties(openWeatherResponse, dto);
			return dto;
	}
}
